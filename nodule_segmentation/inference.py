...
def single_image_inference(image_pth, model_pth, device):
    ...
    # Convertir la máscara predicha a una imagen PIL
    pred_mask_img = transforms.ToPILImage()(pred_mask.squeeze().cpu().detach())

    # Guardar la máscara predicha como un archivo JPG
    pred_mask_img.save("./predictions/prediction.jpg")
    ...
    for i in range(1, 3): 
        ...
        if i == 1:
            plt.imshow(transforms.ToPILImage()(img.squeeze().cpu().detach()), cmap="gray")
...
if __name__ == "__main__":
    SINGLE_IMG_PATH = "./data/test/imagen_008.jpg"
    DATA_PATH = "./data"
    MODEL_PATH = "./saved_model/unet.pth"
